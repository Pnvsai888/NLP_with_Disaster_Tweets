...
# URL of the Twitter X logo image
...
# Add CSS for background image and text color
st.markdown(
    f"""
    <style>
    .stApp {{
    ...
    if tweet_input:
        try:
            if tweet_input.startswith("http"):
                ...
                st.markdown(f'<div class="tweet-container">Tweet: {tweet_text}</div>', unsafe_allow_html=True)
            else:
    ...
    try:
        ...
        if tweets and 'data' in tweets:
            for tweet in tweets['data']:
                ...
                st.markdown(f'<div class="tweet-container">ID: {tweet_id}<br>Tweet: {tweet_text}<br>Time: {tweet_date}</div>', unsafe_allow_html=True)
        else:
